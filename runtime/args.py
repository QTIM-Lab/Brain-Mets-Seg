...
def get_main_args():
    ...
    p.arg("--use-hvd", default=True, help="Whether to use horovod")
    ...
    p.arg("--momentum", type=float_0_1, default=0.99, help="Momentum factor (SGD only)")
    p.arg("--dampening", type=float_0_1, default=0.0, help="Dampening factor (SGD only)")
    p.boolean_flag("--nesterov", default=False, help="Whether to apply nesterov momentum (SGD only)")
    p.boolean_flag("--decoupled", default=True, help="Whether to decouple weight decay (SGD only)")
    
    p.arg(
        "--scheduler",
        type=str,
        default="cosine_annealing",
        choices=["none", "poly", "poly_warmup", "cosine", "cosine_annealing"],
        help="Learning rate scheduler",
    )
    p.arg("--end-learning-rate", type=float, default=0.000001, help="End learning rate")
    p.arg("--factor-decrease-learning-rate", type=float, default=1000, help="Factor by which to decrease the learning rate by")
    p.arg("--warmup-iterations", type=float, default=1000, help="Number of learning rate warmup iterations")
    p.arg("--power", type=float, default=2.5, help="Polynomial factor for polynomial learning rate scheduler")
    p.arg(
    ...
    p.arg("--optimizer", type=str, default="adam", choices=["sgdw", "sgd", "adam", "adamw", "radam"], help="Optimizer")
    ...
    p.arg("--exclude-from-weight-decay", default=False, help="Whether to exclude normalization and bias parameters from weight decay.")
    p.boolean_flag("--use-ema", default=False, help="Whether to use ema of model weights during training")
    p.arg("--ema-momentum", type=float, default=0.999, help="momentum for optional ema")
    p.arg("--ema-overwrite-frequency", type=non_negative_int, default=None, help="How often to update model weights")
    p.boolean_flag(
    ...
    p.flag("--resume-training-best", "--resume_training_best", help="Resume training from the best checkpoint")
    ...
    p.arg("--skip-train-eval", type=non_negative_int, default=0, help="Skip calculating hard dice metric for training data for the first N epochs.")
    p.arg(
        "--steps-per-epoch",
        type=positive_int,

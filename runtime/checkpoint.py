...
class CheckpointManager:
    def __init__(self, ckpt_dir, strategy, variables, step_counter=None, resume_training=False, resume_training_best=False):
        ...
        if "last" in strategy:
            ...
            if resume_training:
                self.ckpt.restore(self.last_manager.latest_checkpoint).expect_partial()
        ...
        if "best" in strategy:
            ...
            if resume_training_best:
                self.ckpt.restore(self.best_manager.latest_checkpoint).expect_partial()
                print('Resuming training from best checkpoint!')
            self.best_metric = None

    @rank_zero_only
    def update_best(self, metric_value, epoch_counter):
        if (
            "best" in self.strategy
            ...
            self.best_manager.save(checkpoint_number=epoch_counter)
    
    @rank_zero_only
    def update_last(self, epoch_counter):
        if "last" in self.strategy:
            self.last_manager.save(checkpoint_number=epoch_counter)
